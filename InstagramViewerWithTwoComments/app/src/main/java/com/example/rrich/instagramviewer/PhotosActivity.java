                ...
                try {
                    ...
                    for (int i=0; i < photosJSON.length(); i++) {
                        ...
                        if (!photoJSON.isNull("location")) {
                            JSONObject location = photoJSON.getJSONObject("location");
                            if (!location.isNull("name")) {
                                photo.locationName = location.getString("name");
                            }
                        ...
                        if (!photoJSON.isNull("comments")) {
                            int commentCount = photoJSON.getJSONObject("comments").getInt("count");
                            int dataLength = photoJSON.getJSONObject("comments").getJSONArray("data").length();
                            if (commentCount > 0) {
                                JSONObject commentInfo = photoJSON.getJSONObject("comments").getJSONArray("data").getJSONObject(dataLength-1);
                                photo.commentUsername1 = commentInfo.getJSONObject("from").getString("username");
                                photo.comment1 = commentInfo.getString("text");
                            }
                            if (commentCount > 1) {
                                JSONObject commentInfo = photoJSON.getJSONObject("comments").getJSONArray("data").getJSONObject(dataLength-2);
                                photo.commentUsername2 = commentInfo.getJSONObject("from").getString("username");
                                photo.comment2 = commentInfo.getString("text");
                            }
                        }
                        photos.add(photo);
                    }
